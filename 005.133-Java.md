...
- JVM 内存管理
    ...
    - **垃圾回收（Garbage Collection）**
        ...
        - 常见的垃圾收集器：Serial、Parallel、CMS、G1等。
            - Serial垃圾收集器（JDK 1.3之前）：单线程；使用复制算法清理年轻代，标记-压缩算法清理老年代。
            - Parallel垃圾收集器（JDK 1.4）：并行；算法同Serial
            - CMS（Concurrent Mark-Sweep）垃圾收集器（JDK 1.4.2）：
                - 顾名思义，并发标记-清除垃圾，主要用于减少老年代的停顿时间
                - 缺点是会产生内存碎片
            - G1（Garbage-First）垃圾收集器（JDK 7 Update 4）
                - 堆划分为多个大小相等的独立区域（Region），每个Region的大小可以在1MB到32MB之间。所有的Region都有各自的用途，包括Eden区、Survivor区和Old区。
                - 年轻代GC（Young GC）：复制算法，将活跃对象从一个区域复制到另一个区域，清理掉不再使用的对象。
                - 老年代GC：并发标记周期（Concurrent Marking Cycle），包括阶段：
                    - 初始标记（Initial Marking）：标记从GC Roots直接可达的对象。这个阶段是短暂的，通常与年轻代GC一起进行。
                    ...
                    - 最终标记（Remark）处理在并发标记阶段发生的引用变化，完成对存活对象的标记。
                    - 筛选和整理（Cleanup）：识别和清理空闲的Region，重新利用这些Region。
                ...
                - 优势
                    - 减少停顿时间、提供可预测的停顿时间、自动调优，适用于对响应时间要求较高的应用程序
                        - 可以通过设置最大停顿时间目标来提供可预测的停顿时间
            - ZGC（Z Garbage Collector）（JDK 11 实验引入）（JDK 13：标记为正式可用）：低停顿时间、并发垃圾收集、区域化内存管理、染色指针
            - Shenandoah（JDK 12 实验引入）（JDK 15：标记为正式可用）：低停顿时间、并发压缩、区域化内存管理
...
- Spring 系列介绍及Spring框架基本
    ...
    - 常见注解
        - @Component：通用 Bean 组件。
        - @Service：标识服务层 Bean。
        - @Repository：标识持久层 Bean。
        - @Controller：标识控制器层 Bean。
        - @Autowired：自动注入 Bean。
        - @Qualifier：指定具体的 Bean 进行注入。
...
- SpringMVC
    ...
    - Spring MVC 常见注解
        ...
        - @PostMapping：映射 HTTP POST 请求到处理方法。
        - @PutMapping：映射 HTTP PUT 请求到处理方法。
        - @DeleteMapping：映射 HTTP DELETE 请求到处理方法。
        - @ResponseBody：将方法的返回值直接作为 HTTP 响应体返回。
        - @ModelAttribute：将方法参数绑定到模型中。
