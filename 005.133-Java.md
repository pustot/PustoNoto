...
- JVM执行步骤
    ...
    - **链接（Linking）**：将加载的类与其他类进行链接，主要包括**验证、准备和解析**三个步骤。
        - 验证：确保类的字节码符合 JVM 规范，不会危害虚拟机。
        - 准备：为类的静态变量分配内存，并将其初始化为默认值。
        - 解析：将常量池中的符号引用替换为直接引用。
            - 这种解析可能在类加载时完成（**静态解析**），也可能在运行时完成（**动态链接**）。动态链接可以实现 Java 的多态性。
...
- 关键组成部分 de JVM
    ...
    - **类加载器（Class Loader）**
        ...
        - 包括三种类加载器：引导（启动）类加载器、扩展类加载器和应用程序类加载器。
            ...
            - 扩展类加载器（Extension ClassLoader）：加载扩展类库（如 ext 目录中的类库）。
            - 应用程序类加载器（Application ClassLoader）：加载应用程序类路径（classpath）中的类。
        - **双亲委派机制**
            - ：指的是类加载器在加载类时，首先将请求委派给父类加载器处理，只有在父类加载器无法加载时，才由自己进行加载
            - 优点
                - 安全性：确保核心 Java 类库不会被自定义类覆盖。
    ...
    - **运行时数据区（Runtime Data Area）**
        ...
        - **方法区（Method Area）**：运行时常量池（Runtime Constant Pool）和静态变量、类信息（类元数据）（反射机制就要用）等。
...
- SpringMVC
    ...
    - 基本概念：模型-视图-控制器（MVC）架构
        ...
        - 视图（View）：负责数据的呈现。
        - 控制器（Controller）：负责处理用户请求，并协调模型和视图。
    - 工作流程
        - 客户端请求：用户通过浏览器发送请求。
        - **前端控制器（DispatcherServlet）**：接收并分发请求。
        - **处理器映射（HandlerMapping）**：找到与请求对应的处理器（Controller）。
        - **处理器适配器（HandlerAdapter）**：调用具体的处理器方法。
        - **处理器（Controller）**：处理请求，调用业务逻辑并返回模型数据。
        - **视图解析器（ViewResolver）**：解析视图名称，找到对应的视图。
        - **视图（View）**：渲染模型数据，生成响应。
        - 返回客户端：视图渲染结果返回给客户端。
    - 关键组件
        - DispatcherServlet：前端控制器，负责请求的接收和分发。
        - Controller：处理请求的核心组件。
        - ModelAndView：封装模型数据和视图名称的对象。
        - ViewResolver：视图解析器，用于将视图名称解析为具体的视图。
        - HandlerMapping：处理器映射，用于将请求映射到具体的处理器。
    - Spring MVC 常见注解
        - @Controller：标识控制器层 Bean。
...
- SpringBoot
    ...
    - 自动配置
        - 自动配置原理：基于条件注解（@Conditional），根据存在的类、属性、方法等条件，自动装配相应的 Bean。
        - 常见自动配置注解
    ...
    - **Spring Boot 自动装配流程**（我也不知道叫配置还是装配，讨论的人很多）
    - 常见注解
        - @SpringBootApplication：组合注解，包含 @Configuration、@EnableAutoConfiguration、@ComponentScan。
